...
async def main():
    ...
    await page.waitForSelector(
        (
            "main > div.css-1u2nsrt > div > div > div.css-8f6za > div > div:nth-child({})"
        ).format(ticker.ticker[get_Ticker])
    )
    await page.click(
        (
            "main > div.css-1u2nsrt > div > div > div.css-8f6za > div > div:nth-child({})"
    ...
    async def scrape(page):
        global results
        results = []
        nextButton = await page.JJ(
            "main > div.css-16g55fu > div > div.css-kwfbf > div > button[aria-label='Next page']:not([disabled])"
        )
        count = 0
        singlePage = True
